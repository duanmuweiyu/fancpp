...
public:
  ...
  virtual cf_Error read(char *buf, size_t size) = 0;
  ...
  inline cf_Error readInt8(int8_t *out) {
    return read((char *)out, 1);
  }
  inline cf_Error readInt16(int16_t *out) {
    cf_Error err = read((char *)out, 2);
  #ifdef CF_ENDIAN_SWAP
    cf_Endian_swap16p(out);
  #endif
    return err;
